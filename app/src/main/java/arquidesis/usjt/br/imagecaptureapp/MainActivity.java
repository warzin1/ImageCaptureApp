...
public class MainActivity extends AppCompatActivity implements ImageInputHelper.ImageActionListener{
    ...
    public void filterApply(View view){
        boolean checked = ((CheckBox) view).isChecked();
        Bitmap bitmap;
        bitmap = ((BitmapDrawable)imagem1.getDrawable()).getBitmap();
        ...
        if(view.getId() == R.id.checkBox3){
            if(checked){
                imagem1.setImageBitmap(imageProcessor.roundCorner(bitmap,60));
            }
        }
        if(view.getId() == R.id.checkBox4){
            if(checked){
                imagem1.setImageBitmap(imageProcessor.rotate(bitmap, 180));
            }
        }
